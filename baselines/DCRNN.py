...
class DiffusionGraphConv(BaseModel):
    ...
    def forward(self, inputs, state, output_size, bias_start=0.0):
        ...
        x = torch.reshape(x, shape=[batch_size * self._num_nodes, input_size * self.num_matrices])  ## flatten
...
class DCGRUCell(BaseModel):
    ...
    def init_hidden(self, batch_size):  # for the first block hidden state input
...
class DCRNNEncoder(BaseModel):
    ...
    def forward(self, inputs, initial_hidden_state):
        ...
        for i_layer in range(self._num_rnn_layers):
            ...
            current_inputs = torch.stack(output_inner, dim=0).to(device)  # seq_len, B, ...
...
class DCGRUDecoder(BaseModel):
    ...
    def forward(self, inputs, initial_hidden_state, teacher_forcing_ratio=0.5):
        """
        inputs: target, iniitial: context
        ...
        # inputs to cell is (batch, num_nodes * input_dim)
        seq_length = inputs.shape[0]  # should be 13 (12 + go symbol)
        batch_size = inputs.shape[1]
...
class DCRNNModel(BaseModel):
    def __init__(self, adj_mat, args):
        ...
        self._enc_input_dim = args.enc_input_dim
        self._num_nodes = args.num_nodes  # should be 207
        ...
        self._output_dim = args.output_dim  # should be 1
        self.device = args.device

        # specify a GO symbol as the start of the decoder
        # self.GO_Symbol = torch.zeros(1, batch_size, num_nodes * self._output_dim, 1).to(device)  #origin
        self.GO_Symbol = torch.zeros(1, args.batch_size, args.num_nodes, 3).to(args.device)

        self.encoder = DCRNNEncoder(input_dim=args.enc_input_dim, adj_mat=adj_mat,
                                    max_diffusion_step=args.max_diffusion_step,
                                    hid_dim=args.rnn_units, num_nodes=args.num_nodes,
                                    num_rnn_layers=args.num_rnn_layers, filter_type=args.filter_type)
        self.decoder = DCGRUDecoder(input_dim=args.dec_input_dim,
                                    adj_mat=adj_mat, max_diffusion_step=args.max_diffusion_step,
                                    num_nodes=args.num_nodes, hid_dim=args.rnn_units,
    ...
    def forward(self, source, target, teacher_forcing_ratio):
        ...
        target = torch.cat([self.GO_Symbol, target], dim=0) # cat in seq_len dim
        ...
        outputs = outputs[1:, :, :].swapaxes(0, 1) # (seq_length, batch_size, num_nodes*output_dim)  (12, 64, 207*1)

        # outputs = torch.transpose(outputs[1:, :, :].view(12, model.batch_size, model.num_nodes,
        #                                          model.output_dim), 0, 1)  # back to (50, 12, 207, 1)
        return outputs.reshape([-1, source.shape[0], self._num_nodes, self._enc_input_dim])   #[batch, seq_len, byn_nodes, output_dim]
