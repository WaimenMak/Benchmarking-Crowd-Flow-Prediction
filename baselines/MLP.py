...
file_handler = logging.FileHandler("./result/train MLP.log")
...
def main(args):
    ...
    if args.mode == "in-sample":
        ...
        data = load_dataset("./dataset", batch_size=args.batch_size, test_batch_size=args.batch_size)
    elif args.mode == "ood":
        # train = np.load("./ood_dataset/train.npz")
        # val = np.load("./ood_dataset/val.npz")
        # test = np.load("./ood_dataset/test.npz")
        data = load_dataset("./ood_dataset", batch_size=args.batch_size, test_batch_size=args.batch_size)
    else:
        raise ValueError
